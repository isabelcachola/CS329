...
d <- data.frame("winner") %>%
  dplyr::mutate(
    mgmt_prof=cor(data$winner,management_professional_and_related_occupations),
    service=cor(data$winner,service_occupations),
    sales_office=cor(data$winner,sales_and_office_occupations),
    agriculture=cor(data$winner,farming_fishing_and_forestry_occupations),
    construction=cor(data$winner,construction_extraction_maintenance_and_repair_occupations),
    transportation=cor(data$winner,production_transportation_and_material_moving_occupations)
  )
...
mean(n.knn.preds==pop_vote_winner[!train_cond])
```
