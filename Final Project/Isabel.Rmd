...
Let's use exhaustive subset selection for predictors of At Risk Funding.
```{r,include=FALSE}
...
```{r}
regfit.full=regsubsets(at_risk_budget~.,data=df[train,])
...
According to subset selection, the optimal predictors are the following: 

- **Number of English Language Learners**
- **Number of Homeless/Foster Students:**
- **Number of students one year older**
...
Let's try it out:
```{r}
subset.fit = lm(at_risk_budget~english_language_learners+homeless_foster+one_year_older+total_projected_enrollment+title_funding+perc_at_risk, data = df[train,])
summary(subset.fit)

ggplot(data=df_at_risk16,mapping = aes(y=at_risk_budget,x=english_language_learners+homeless_foster+one_year_older+total_projected_enrollment+title_funding+perc_at_risk)) +
  geom_point() + xlab("X") +
  geom_smooth(method='lm', colour="red") +
  scale_y_continuous(label=scales::comma)
```

This produces a relatively high R2 and low p-value on the training set.
...
The random forest produces a lower $$R^2$$ than the linear model.
...
pred.subset <- predict(subset.fit, df[-train,])
subset.err <- mean ((pred.subset - df[-train,])^2)/m
...
errors = data.frame(method=c("Random Forest","Boosting","Linear Regression","Support Vector Machine","Subset Selection","Ridge Regression","Lasso"),
                    err = c(rf.err, boost.err,lm.err,svm.err,subset.err, ridge.err,lasso.err))
errors
```
